...
//     the measurement is T_meas = T_i_j (frame j expressed in frame i), so that for the
//     estimates T_i_j = T_i^-1 T_j and the residual is r = Log(T_meas^-1 * (T_i^-1 T_j)).
...
//   r = Log(T_meas^-1 * (T_i^-1 * T_j))  (a 6-vector, zero when the estimate matches).
...
// With r the residual above, the published (Barfoot / GTSAM BetweenFactor) forms are
//   dr/d(delta_i) = -J_r^-1(r) * Ad_{T_j^-1 T_i},   dr/d(delta_j) = J_r^-1(r).
// The minus sign on the i-side and the adjoint argument T_j^-1 T_i are the classic hand-rolled
// pose-graph bug; the numerical-vs-analytic test gates them. Returns (J_i, J_j).
...
// measurement meas[k] and information info[k]. Each iteration assembles the normal equations
//   H = sum_k J_k^T Omega_k J_k,   g = sum_k J_k^T Omega_k r_k
// over the 6 DOF per pose, fixes the gauge by anchoring pose 0 (its block is held fixed),
// solves H delta = -g for the free poses, and retracts T <- T * se3_exp(delta). Returns the
// optimized poses and the final total weighted squared residual.
...
// cheap). With n_pose the number of leading pose columns, marginalize the landmarks:
//   H_red = H_pp - H_pl H_ll^-1 H_lp,   b_red = b_p - H_pl H_ll^-1 b_l,
// solve H_red delta_p = b_red, then back-substitute delta_l = H_ll^-1 (b_l - H_lp delta_p).
