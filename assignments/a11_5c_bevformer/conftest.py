...
student edits. The owned mechanism file bevformer.py is NOT imported bare - it comes through
nanovision.bevformer, which nanovision/_student.py routes to the top-level (default) or
solution/ copy keyed on NANOVISION_IMPL. The top-level dir is always on the path (config.py
lives only there); solution/ is added first under NANOVISION_IMPL=solution so its bevformer.py
