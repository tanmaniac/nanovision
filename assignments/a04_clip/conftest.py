...
Local files (model.py, losses.py, inference.py, config.py) are imported with a bare
name. By default they resolve to the top-level files the student edits; with
NANOVISION_IMPL=solution they resolve to solution/. config.py and model.py live only
at the top level (provided), so the top-level dir is always on the path too.
"""
