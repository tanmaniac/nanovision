...
provided encoder and a light decoder is given. The reference lives in
`solution/mae.py` (read it if you get stuck).
...
def append_mask_tokens(x_enc: Tensor, ids_restore: Tensor, mask_token: Tensor) -> Tensor:
    ...
    concatenating [visible; mask] in shuffled order, then gathering by ids_restore so
    position i holds either its encoded visible token or a mask token, in original
...
def mae_loss(pred: Tensor, target: Tensor, mask: Tensor) -> Tensor:
    ...
    (B, N) with 1 on masked patches. Compute the mean squared error per patch (mean
