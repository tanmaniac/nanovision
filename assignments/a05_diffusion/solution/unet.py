...
down/up sampling with skip connections). The two diffusion-specific pieces are the holes
in the student copy: `timestep_embedding` (the sinusoidal embedding from the transformer,
now indexed by the scalar diffusion timestep instead of sequence position) and the AdaGN
time-injection line in the ResNet block (how the timestep+class signal enters the
network; A7's adaLN-Zero generalizes exactly this). Everything else is provided so a buggy
network cannot make the diffusion tests flaky.
...
def timestep_embedding(t: Tensor, dim: int) -> Tensor:
    ...
    diffusion timestep value rather than a sequence position.
    """
    half = dim // 2
...
class ResBlock(nn.Module):
    ...
    def forward(self, x: Tensor, temb: Tensor) -> Tensor:
        ...
        # AdaGN time injection: project the time+class embedding to this block's channels
        # and add it as a per-channel shift (broadcast over H, W).
