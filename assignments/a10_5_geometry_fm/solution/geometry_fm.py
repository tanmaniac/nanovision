...
def depth_to_pointmap(depth: Tensor, K: Tensor) -> Tensor:
    ...
    vs, us = torch.meshgrid(
        ...
        indexing="ij",
    )  # (H, W) each: vs = row index, us = column index
    px = torch.stack([us, vs], dim=-1)              # (H, W, 2) -> (u, v)
    px = px.expand(B, H, W, 2)                       # (B, H, W, 2)
    return unproject(px, depth, K)                   # (B, H, W, 3)
...
def reproject_pointmap(pts_cam1: Tensor, T_1to2: Tensor, K: Tensor) -> Tensor:
    ...
    """
    pts_cam2 = apply_transform(T_1to2, pts_cam1)     # (B, H, W, 3)
    return project_points(pts_cam2, K)               # (B, H, W, 2)
