...
student edits. predict.py is also assignment-LOCAL here (nothing imports it, so there is no
...
there); solution/ is added first under NANOVISION_IMPL=solution so its predict.py shadows the
holed top-level one.
"""
...
# _impl first (higher priority) so solution/predict.py shadows the holed top-level one;
