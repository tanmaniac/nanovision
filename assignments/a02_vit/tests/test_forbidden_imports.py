...
Scans every .py in the A2 solution and the shared-library primitives module for
the high-level shortcuts the learner is supposed to build by hand: prebuilt
attention/transformer modules, fused SDPA, timm/torchvision.models, and
nn.LayerNorm (the course builds LayerNorm from scratch). Comments and string
literals are stripped first (via tokenize) so merely naming a forbidden symbol in a
docstring does not trip the test; only real code usage does.
