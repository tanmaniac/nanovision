...
def box_xyxy_to_cxcywh(b: Tensor) -> Tensor:
    ...
    The inverse of box_cxcywh_to_xyxy: center is the midpoint of the two corners, width and
    height are the corner differences.
    """
...
def generalized_iou(boxes1: Tensor, boxes2: Tensor, eps: float = 1e-7) -> Tensor:
    ...
    enclosing A and B. Steps:
      1. Convert both sets cxcywh -> xyxy and compute each box's area.
      2. Intersection box: top-left = elementwise max of the two top-lefts, bottom-right =
         elementwise min of the two bottom-rights; clamp width/height to >= 0 so disjoint
         boxes give zero intersection, not a spurious negative one. inter = w * h.
      3. union = area1 + area2 - inter; IoU = inter / (union + eps).
      4. Enclosing box C: top-left = min of top-lefts, bottom-right = max of bottom-rights;
         area_c = its w * h.
      5. GIoU = IoU - (area_c - union) / (area_c + eps).
    Add eps to BOTH denominators (the union and area_c) so zero-area degenerate boxes do not
    divide by zero. The result is differentiable, so the same function is reused in the box
    loss.
    """
