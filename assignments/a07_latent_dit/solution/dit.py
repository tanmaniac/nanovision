...
def modulate(x: Tensor, shift: Tensor, scale: Tensor) -> Tensor:
    ...
    x is (B, N, d); shift and scale arrive as (B, d) and are unsqueezed to (B, 1, d) here so
    they broadcast over the N token axis. Without the unsqueeze the (B, d) tensors do not
    broadcast against (B, N, d).
    """
...
def patchify(z: Tensor, p: int) -> Tensor:
    ...
    B, C, H, W = z.shape
    nh, nw = H // p, W // p
    z = z.reshape(B, C, nh, p, nw, p)            # split H, W into (patch, within) pairs
    z = z.permute(0, 2, 4, 1, 3, 5)              # (B, nh, nw, C, p, p)
    return z.reshape(B, nh * nw, C * p * p)
...
def unpatchify(tokens: Tensor, p: int, C: int, H: int, W: int) -> Tensor:
    ...
    B = tokens.shape[0]
    nh, nw = H // p, W // p
    z = tokens.reshape(B, nh, nw, C, p, p)
    z = z.permute(0, 3, 1, 4, 2, 5)              # (B, C, nh, p, nw, p)
