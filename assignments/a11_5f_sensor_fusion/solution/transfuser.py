...
Reference (filled) implementation. The holed top-level ``transfuser.py`` carries the identical
docstrings; only the hole body below differs. This is an assignment-LOCAL module, imported by
bare name (``from transfuser import TransFuserBlock``); the attention block it builds on comes
through ``nanovision.transformer``.
...
class TransFuserBlock(nn.Module):
    ...
    def forward(self, cam_tokens: Tensor, lidar_tokens: Tensor) -> tuple[Tensor, Tensor]:
        ...
        """
        Sc = cam_tokens.shape[1]
        x = torch.cat([cam_tokens, lidar_tokens], dim=1)   # (B, Sc + Sl, dim)
        y = self.block(x)                                  # (B, Sc + Sl, dim)
        cam_out = cam_tokens + y[:, :Sc]
        lidar_out = lidar_tokens + y[:, Sc:]
