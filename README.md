...
PnP, RANSAC, and the two-view front-end), `a14_4_icp` (point-to-point and point-to-plane
registration), and `a14_5_factor_graph` (pose-graph optimization and the bundle-adjustment
Schur complement) are built. The module's mechanism assignments are complete; a reading-only
`notes/` writeup (IMU pre-integration, VIO, loop-closure detection, and the bridge to the
learned geometry models) is the remaining piece.
